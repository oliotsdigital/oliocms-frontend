...
interface BrandTableProps {
  brandsList: Brand[];
  onDeleteBrand: (id: number | string) => void;
}
  ...
  return (
    <div className="lg:col-span-7 glass-panel rounded-2xl overflow-hidden">
      <div className="p-3.5 border-b border-slate-200/40 dark:border-slate-800/40 font-semibold text-xs text-slate-900 dark:text-white flex items-center justify-between">
        <span>Created Brands ({brandsList.length})</span>
      </div>
          ...
          <tbody className="divide-y divide-slate-200/30 dark:divide-slate-800/30 text-xs">
            {brandsList.length === 0 ? (
              ...
              brandsList.map((brand) => (
                <tr key={brand.id} className="hover:bg-slate-200/20 dark:hover:bg-slate-800/20 transition">
                  ...
                  <td className="py-2.5 px-3 text-slate-500 font-mono text-[11px]">{brand.slug}</td>
                  <td className="py-2.5 px-3 text-slate-500">{brand.parent}</td>
