    ...
    >
      ...
      <div className="min-w-0 flex-1">
        ...
        <div className="mt-1 flex items-center gap-1.5 flex-wrap">
          ...
          <span
            className={`px-1.5 py-0.5 rounded-md text-[9px] font-bold inline-flex items-center gap-1 ${
              col.is_public !== false
                ? "bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 border border-emerald-500/20"
                : "bg-amber-500/10 text-amber-600 dark:text-amber-400 border border-amber-500/20"
            }`}
          >
            <i className={`fa-solid ${col.is_public !== false ? "fa-globe" : "fa-lock"} text-[8px]`}></i>
            {col.is_public !== false ? "Public" : "Private"}
          </span>
        </div>
      </div>
            ...
            <>
              ...
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-4 border-b border-slate-200/40 dark:border-slate-800/40">
                <div>
                  <div className="flex items-center gap-2.5 flex-wrap">
                    <div
                      ...
                      className="w-9 h-9 rounded-xl bg-brand-500/10 border border-brand-500/20 flex items-center justify-center cursor-pointer hover:bg-brand-500/20 transition group shrink-0"
                    ...
                    />
                    <span
                      className={`px-2.5 py-0.5 rounded-full text-[10px] font-bold inline-flex items-center gap-1.5 shrink-0 ${
                        selectedCollection.is_public !== false
                          ? "bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 border border-emerald-500/20"
                          : "bg-amber-500/10 text-amber-600 dark:text-amber-400 border border-amber-500/20"
                      }`}
                    >
                      <i className={`fa-solid ${selectedCollection.is_public !== false ? "fa-globe" : "fa-lock"} text-[9px]`}></i>
                      {selectedCollection.is_public !== false ? "Public" : "Private"}
                    </span>
                  </div>
                </div>
                ...
                <div className="flex flex-wrap items-center gap-2.5">
                  ...
                  {selectedCollection.is_public === false ? (
                    <span
                      className="px-4 py-2.5 rounded-xl bg-slate-100 dark:bg-slate-800 text-slate-400 dark:text-slate-500 font-bold text-xs flex items-center gap-2 border border-slate-200 dark:border-slate-700 cursor-not-allowed opacity-50 select-none shadow-sm"
                    ...
                    <Link
                      href={`/collections/${selectedCollection.id}/apis`}
                      className="px-4 py-2.5 rounded-xl bg-brand-500/10 hover:bg-brand-500 text-brand-500 hover:text-white font-bold text-xs transition flex items-center gap-2 border border-brand-500/20 shadow-sm"
